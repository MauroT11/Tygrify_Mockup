...
**2. Magic link sign-in, no passwords.** The customer enters their email, receives a link, and is signed in. No passwords stored, no reset flow, no forgotten-password phone calls to a one-man business, and a much smaller security surface. Optionally add SMS one-time codes later given how phone-heavy this audience is.
...
**`/account/sign-in`** - Email entry, magic link sent, plain confirmation screen.
...
**Flow:** Delete my account → plain-English explanation of what goes and what stays → re-authenticate via magic link (proves it is the account holder, not someone on an unlocked phone) → confirm → immediate action, with a confirmation email sent afterwards.
...
**Linking an anonymous enquiry to an account later.** Store enquiries with an email and a null `user_id`. When someone signs in with a matching email, backfill `user_id` on their enquiries. This is safe specifically because magic-link sign-in proves control of that email address - it would not be safe with password auth and no verification step.
