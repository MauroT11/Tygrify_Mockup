...
Every service page on the public site is generated from one record here. This is the screen the client will actually use most often after enquiries, because prices and turnaround times change.

**Where it lives depends on the platform question.** With a CMS, this is a content type and there is nothing bespoke to build. With Supabase and no CMS - which is where the stack is currently heading - it is a `services` table plus a straightforward list-and-edit screen. Either way the content model below is the same, and it needs agreeing before either route starts.

**Fields per service:**

| Field | Notes |
| --- | --- |
| Customer-facing name | Becomes the H1 and the nav label - "Bumper Scuff & Crack Repair", not internal jargon |
| Slug | Editable, but warn on change - it breaks existing links and search rankings |
| Category | Body & paint / Cosmetic & aftermarket / Security & customisation. Drives grouping on the services overview |
| One-line summary | Listing pages and search results |
| Full description | Plain English, jargon defined on first use |
| Who it's for | Per the service page template |
| Typical turnaround | Free text - the client's answers vary by job, so a structured field would fight the reality |
| Price type | From / Fixed / Quote on inspection |
| Price amount | Only used where price type is From or Fixed |
| Publish price | **Separate toggle, defaults off** - see below |
| Related search terms | The client supplied long lists of what customers call each job. Useful for on-page copy and internal search. Not a keywords meta tag |
| Gallery filter tag | Which gallery images belong to this service |
| Meta title / meta description | Falls back to name and summary if left blank |
| Status | Draft / Published |
| Sort order | Within its category |

**The publish-price toggle is a guardrail, not a convenience.** The client agreed to show indicative prices on cosmetic work only, with structural and collision work staying quote-only. Make that hard to get wrong: default the toggle off, and show a warning when it is switched on for anything in Body & paint. Without this, one edit on a phone puts a "from £" figure on a collision repair page and creates an expectation the client cannot meet.

**Do not add a per-service guarantee field yet.** The guarantee is unresolved - the client's answer was lifetime or six months depending on the repair, which is not publishable as written. Once he has given a tiered version in plain terms, this becomes a field. Until then it should not be editable, because an editable field invites someone to fill it in.

### Security and customisation services

Trackers, Ghost immobilisers and CAN-BUS work sit in the same table but behave differently enough to need their own extra fields. The client specifically asked for trackers and Ghost immobilisers to have their own pages rather than being folded into one security page.

**Additional fields for this category:**

| Field | Notes |
| --- | --- |
| Vehicle compatibility | CAN-BUS work is make and model dependent. Needs to be sayable without promising universal fitment |
| Fitting location | Workshop or mobile - the business started in mobile repairs, so this may vary |
| Ongoing costs | See the open question below - present but empty until answered |
| Insurance recognition | See the open question below |

**Two things to ask the client before these pages are written:**

1. **Do the tracker prices include a subscription?** Tracking products commonly carry an ongoing monitoring or subscription fee. The client gave "trackers from £350" with no indication of whether that is fitting only. Publishing a fitting-only price as if it were the total is the kind of thing that generates complaints. Do not assume either way - ask.
2. **Is any of the security work insurer-recognised?** Approval status affects whether a customer gets an insurance discount, and it is one of the main reasons people buy these products. It is also an accreditation-style claim, so it falls under the existing rule: nothing published without documentation in hand.

### Security services break the quote flow - not just an admin gap

Worth raising separately, because it affects v1 and not only this phase. The quote flow is built around body type → panel selection → damage detail. **None of that applies to someone wanting a Ghost immobiliser fitted.** There is no damaged panel to click.

As currently specced, a tracker enquiry either forces the customer through an irrelevant damage selector or has nowhere to go at all. The fix is a fork at the top of the quote form - "What do you need?" → damage repair, or security and customisation - with the second branch collecting registration, service wanted, and preferred dates, and skipping the selector entirely. Cheap to add now, awkward to retrofit.

### `/admin/gallery`

- Upload, with the redaction pipeline applied before anything is visible
- Assign each image to a service, and mark before/after pairs
- **Approve before publish** - the human review gate described in the redaction section. Nothing reaches a public page without it
- Reorder, caption, unpublish

### `/admin/business-info`

Single form, no sub-pages: trading name, addresses, phone, enquiry email, opening hours, areas covered, social links, footer legal details. Low frequency of use but high cost when wrong, since it feeds the footer and the contact page site-wide.
